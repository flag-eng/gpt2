                    ...
                    if (overall_step + 1) % args.log_step == 0:
                        logger.info(
                            "batch {} of epoch {}, loss {}, accuracy {}".format(batch_idx + 1, epoch + 1, loss,
                                                                                accuracy))
                    tb_writer.add_scalar('train/loss', loss.item(), overall_step)
                    tb_writer.add_scalar('train/acc', accuracy.item(), overall_step)
...
def evaluate(model, device, test_list, multi_gpu, args):
    ...
    with torch.no_grad():
        for batch_idx, input_ids in enumerate(test_dataloader):
            ...
            tb_writer.add_scalar('valid/loss', loss.item(), overall_step)
            tb_writer.add_scalar('valid/acc', accuracy.item(), overall_step)
            overall_step += 1
...
def main():
  ...
  # 初始化tokenizer
    ...
    logger.info("data_list num = {}".format(len(data_list)))
